...
Error LogProvider::Init(logger::LogReaderItf& logReader)
{
...
Error LogProvider::HandleLogRequest(const cloudprotocol::RequestLog& request)
{
    ...
    auto logEntry = MakeUnique<logger::LogEntry>(&mAllocator);
    ...
    while (true) {
        ...
        if (auto err = HandleLogRequest(mLogRequests.Front()); !err.IsNone()) {
            if (auto sendErr = SendErrorLog(mLogRequests.Front().mLogID, err); !sendErr.IsNone()) {
                LOG_ERR() << "Failed to send error log: " << sendErr;
            }
        }
...
bool LogProvider::SkipLogEntry(const logger::LogEntry& logEntry, const cloudprotocol::RequestLog& request)
{
