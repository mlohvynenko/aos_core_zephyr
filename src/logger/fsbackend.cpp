...
uint8_t __aligned(4) sLogBuffer[cLogEntryLen];
...
const struct log_backend_api aos_log_backend_fs_api = {
    .process    = Process,
...
LOG_BACKEND_DEFINE(aos_log_backend_fs, aos_log_backend_fs_api, false);

const struct log_backend* log_backend_aos_get(void)
{
    return &aos_log_backend_fs;
}
...
void SetLogCustomTimestamp()
{
    log_custom_timestamp_set(UTCTimestamp);
}
